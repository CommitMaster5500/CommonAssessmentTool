...
class ClientService:
    ...
    def get_clients(db: Session, skip: int = 0, limit: int = 50):
        ...
        Default shows first 50 clients, which means you'd need 3 pages
        for 150 records.
        """
        if skip < 0:
            raise HTTPException(
    ...
    def get_clients_by_success_rate(db: Session, min_rate: int = 70):
        """Get clients with success rate at or above the specified
        percentage."""
    ...
    def update_client_services(
        ...
        """Update a client's services and outcomes for a specific case
        worker."""
        client_case = (
            db.query(ClientCase)
